...
- (NSInteger)tableView:(UITableView *)tableView typeForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.tableViewCellArray != nil){
        if (respondsSel(self.smDataSource ,@selector(tableView:typeForRowAtIndexPath:))) {
            type = [self.smDataSource tableView:tableView typeForRowAtIndexPath:indexPath];
        }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    //自定义
    if(respondsSel(self.smDataSource,@selector(numberOfSectionsInTableView:))){
        return [self.smDataSource numberOfSectionsInTableView:tableView];
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //自定义
    if(respondsSel(self.smDataSource,@selector(tableView:numberOfRowsInSection:))){
        return [self.smDataSource tableView:tableView numberOfRowsInSection:section];
    }
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView {
    //自定义
    if(respondsSel(self.smDataSource , @selector(sectionIndexTitlesForTableView:))){
        return [self.smDataSource sectionIndexTitlesForTableView:tableView];
    }
    return self.sectionIndexTitles;
}

- (NSInteger)tableView:(UITableView *)tableView sectionForSectionIndexTitle:(NSString *)title atIndex:(NSInteger)index {
    if (respondsSel(self.smDataSource,@selector(tableView:sectionForSectionIndexTitle:atIndex:))) {
        return [self.smDataSource tableView:tableView sectionForSectionIndexTitle:title atIndex:index];
    }
    return -1;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    //自定义
    if(respondsSel(self.smDataSource,@selector(tableView:cellForRowAtIndexPath:))){
        return [self.smDataSource tableView:tableView cellForRowAtIndexPath:indexPath];
    }
    id dataInfo = [self dataInfoforCellatTableView:tableView IndexPath:indexPath];
    ...
    if(respondsSel(self.smDelegate,@selector(tableView:accessoryTypeForRowWithIndexPath:))){
    #pragma clang diagnostic push
    #pragma clang diagnostic ignored "-Wdeprecated-declarations"
        cell.accessoryType = [self.smDelegate tableView:tableView accessoryTypeForRowWithIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView cellStyleForRowAtIndexPath:(UITableViewCell *)cell{
    if(respondsSel(self.smDataSource,@selector(tableView:cellStyleForRowAtIndexPath:))){
        [self.smDataSource tableView:tableView cellStyleForRowAtIndexPath:cell];
    }
}
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section{
    if(respondsSel(self.smDataSource,@selector(tableView:titleForHeaderInSection:))){
        return [self.smDataSource tableView:tableView titleForHeaderInSection:section];
    }else{
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section{
    if(respondsSel(self.smDataSource,@selector(tableView:titleForFooterInSection:))){
        return [self.smDataSource tableView:tableView titleForFooterInSection:section];
    }
    return nil;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if(respondsSel(self.smDelegate,@selector(tableView:didSelectRowAtIndexPath:))){
        return [self.smDelegate tableView:tableView didSelectRowAtIndexPath:indexPath];
    }else{
        id dataInfo = [self dataInfoforCellatTableView:tableView IndexPath:indexPath];
...
//        }];
    ...
    }else if(respondsSel(self.smDelegate,@selector(tableView:heightForRowAtIndexPath:))){
        return [self.smDelegate tableView:tableView heightForRowAtIndexPath:indexPath];
    }else{
        //将计算高度的方法交给cell来处理
        //生成cellid
        id dataInfo = [self dataInfoforCellatTableView:tableView IndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if(respondsSel(self.smDelegate,@selector(tableView:heightForHeaderInSection:))){
        return [self.smDelegate tableView:tableView heightForHeaderInSection:section];
    }else if(respondsSel(self.smDataSource,@selector(tableView:titleForHeaderInSection:))){
        NSString *title = [self.smDataSource tableView:tableView titleForHeaderInSection:section];
        if(title != nil){
            return -1.0f;
        }
    }else{
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if(respondsSel(self.smDelegate,@selector(tableView:heightForFooterInSection:))){
        return [self.smDelegate tableView:tableView heightForFooterInSection:section];
    }else if(respondsSel(self.smDataSource,@selector(tableView:titleForFooterInSection:))){
        NSString *title = [self.smDataSource tableView:tableView titleForFooterInSection:section];
        if(title != nil){
            return -1.0f;
        }
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if(respondsSel(self.smDelegate,@selector(tableView:viewForHeaderInSection:))){
        return [self.smDelegate tableView:tableView viewForHeaderInSection:section];
    }else if(respondsSel(self.smDataSource,@selector(tableView:titleForHeaderInSection:))){
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if(respondsSel(self.smDelegate,@selector(tableView:viewForFooterInSection:))){
      return [self.smDelegate tableView:tableView viewForFooterInSection:section];
    }
    return nil;
}
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if(respondsSel(self.smDelegate,@selector(tableView:willDisplayCell:forRowAtIndexPath:))){
        return [self.smDelegate tableView:tableView willDisplayCell:cell forRowAtIndexPath:indexPath];
    }
}
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0) {
    if(respondsSel(self.smDelegate ,@selector(tableView:willDisplayHeaderView:forSection:))){
        return [self.smDelegate tableView:tableView willDisplayHeaderView:view forSection:section];
    }
}
- (void)tableView:(UITableView *)tableView willDisplayFooterView:(UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0) {
    if(respondsSel(self.smDelegate,@selector(tableView:willDisplayFooterView:forSection:))){
        return [self.smDelegate tableView:tableView willDisplayFooterView:view forSection:section];
    }
}
- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath*)indexPath NS_AVAILABLE_IOS(6_0) {
    if(respondsSel(self.smDelegate,@selector(tableView:didEndDisplayingCell:forRowAtIndexPath:))){
        return [self.smDelegate tableView:tableView didEndDisplayingCell:cell forRowAtIndexPath:indexPath];
    }
}
- (void)tableView:(UITableView *)tableView didEndDisplayingHeaderView:(UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0) {
    if(respondsSel(self.smDelegate,@selector(tableView:didEndDisplayingHeaderView:forSection:))){
        return [self.smDelegate tableView:tableView didEndDisplayingHeaderView:view forSection:section];
    }
}
- (void)tableView:(UITableView *)tableView didEndDisplayingFooterView:(UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0) {
    if(respondsSel(self.smDelegate,@selector(tableView:didEndDisplayingFooterView:forSection:))){
        return [self.smDelegate tableView:tableView didEndDisplayingFooterView:view forSection:section];
    }
}
...
- (void)scrollViewDidScroll:(UITableView *)tableView{
    if(isTableView(tableView) && respondsSel(self.smDelegate,@selector(scrollViewDidScroll:))){
        [self.smDelegate scrollViewDidScroll:tableView];
    }
}

- (void)scrollViewWillBeginDragging:(UITableView *)tableView {
    if(isTableView(tableView) && respondsSel(self.smDelegate,@selector(scrollViewWillBeginDragging:))){
        [self.smDelegate scrollViewWillBeginDragging:tableView];
    }
}
- (void)scrollViewWillEndDragging:(UITableView *)tableView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset NS_AVAILABLE_IOS(5_0) {
    if(isTableView(tableView) && respondsSel(self.smDelegate,@selector(scrollViewWillEndDragging:withVelocity:targetContentOffset:))){
        [self.smDelegate scrollViewWillEndDragging:tableView withVelocity:velocity targetContentOffset:targetContentOffset];
    }
}

- (void)scrollViewDidEndDragging:(UITableView *)tableView willDecelerate:(BOOL)decelerate {
    if(isTableView(tableView) && respondsSel(self.smDelegate ,@selector(scrollViewDidEndDragging:willDecelerate:))){
        [self.smDelegate scrollViewDidEndDragging:tableView willDecelerate:decelerate];
    }
}

- (void)scrollViewWillBeginDecelerating:(UITableView *)tableView {
    if(isTableView(tableView) && respondsSel(self.smDelegate ,@selector(scrollViewWillBeginDecelerating:))){
        [self.smDelegate scrollViewWillBeginDecelerating:tableView];
    }
}

- (void)scrollViewDidEndDecelerating:(UITableView *)tableView{
    if(isTableView(tableView) && respondsSel(self.smDelegate ,@selector(scrollViewDidEndDecelerating:))){
        [self.smDelegate scrollViewDidEndDecelerating:tableView];
    }
}
